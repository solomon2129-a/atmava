...
export default function LoginPage() {
  ...
  const { user, signInWithEmail, signInWithGoogle, signInWithApple, loading: authLoading } = useAuth();
      ...
      <motion.div className="w-full max-w-md relative z-10" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9 }}>
        <div className="p-10 rounded-2xl" style={{ background: "rgba(246,244,239,0.85)", border: "1px solid #D4CCBF", backdropFilter: "blur(20px)", boxShadow: "0 24px 60px rgba(44,43,41,0.07)" }}>
          <div className="text-center mb-10">
            <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "2.2rem", fontWeight: 300, color: "#2C2B29" }}>Welcome back</h1>
            <p className="text-sm mt-2" style={{ color: "#7A7771" }}>Return to your practice</p>
          </div>

          {/* Social */}
          <div className="space-y-3 mb-7">
            <motion.button onClick={handleGoogle} disabled={loading} className="w-full py-3.5 rounded-xl text-sm flex items-center justify-center gap-3" style={{ border: "1px solid #D4CCBF", background: "#F6F4EF", color: "#2C2B29" }} whileHover={{ borderColor: "#5C6B57", background: "#E8E1D6" }} whileTap={{ scale: 0.98 }}>
            ...
            <motion.button onClick={handleApple} disabled={loading} className="w-full py-3.5 rounded-xl text-sm flex items-center justify-center gap-3" style={{ border: "1px solid #D4CCBF", background: "#F6F4EF", color: "#2C2B29" }} whileHover={{ borderColor: "#5C6B57", background: "#E8E1D6" }} whileTap={{ scale: 0.98 }}>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} onFocus={() => setFocusedField("email")} onBlur={() => setFocusedField(null)} className="w-full px-4 py-3.5 rounded-xl text-sm outline-none transition-all duration-300" style={inputStyle("email")} placeholder="you@example.com" />
            </div>
            <div>
              <div className="flex justify-between mb-2">
                <label className="text-xs tracking-widest uppercase" style={{ color: "#7A7771" }}>Password</label>
                <Link href="/auth/forgot-password"><span className="text-xs" style={{ color: "#5C6B57" }}>Forgot password?</span></Link>
              </div>
              <div className="relative">
                <input type={showPass ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} onFocus={() => setFocusedField("password")} onBlur={() => setFocusedField(null)} className="w-full px-4 py-3.5 pr-12 rounded-xl text-sm outline-none transition-all duration-300" style={inputStyle("password")} placeholder="••••••••" />
            ...
            <AnimatePresence>
              {error && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0 }} className="text-sm px-4 py-3 rounded-xl" style={{ background: "rgba(200,80,80,0.06)", color: "#c04040", border: "1px solid rgba(200,80,80,0.15)" }}>
                  {error}
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <motion.button type="submit" disabled={loading} className="w-full py-4 rounded-xl text-sm tracking-[0.14em] uppercase flex items-center justify-center gap-3 mt-2" style={{ background: "#5C6B57", color: "#F6F4EF" }} whileHover={{ background: "#4A5645", boxShadow: "0 8px 24px rgba(92,107,87,0.25)" }} whileTap={{ scale: 0.98 }}>
              {loading ? <motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }}><Loader size={16} /></motion.div> : "Enter"}
            </motion.button>
          </form>

          <div className="mt-8 text-center">
            <p className="text-sm" style={{ color: "#7A7771" }}>New to Atmava?{" "}
              <Link href="/auth/signup"><motion.span className="underline" style={{ color: "#5C6B57" }} whileHover={{ opacity: 0.7 }}>Begin your journey</motion.span></Link>
            </p>
          </div>
        </div>
      </motion.div>
